...
options(scipen = 999)
setwd("/Users/sameernair-desai/Desktop/COVID_Dominance")
```
...
Confirmed_Long <- pivot_longer(Initial_Confirmed_Wide, cols = c(5:149), names_to = "Dates", values_to = "Confirmed_Cases")
...
Deceased_Long <- pivot_longer(Initial_Deceased_Wide, cols = c(5:149), names_to = "Dates", values_to = "Deceased")
require(reshape)
...
Temporary_Data_Country <- Temporary_Data_Country %>%
  ...
  dplyr::summarise(Total_Cases_Country = sum(Confirmed_Cases), Total_Deceased_Country = sum(Deceased))
...
Government_Responses <- read_csv("/Users/sameernair-desai/Desktop/COVID_Dominance/data/Government_Responses.csv")
...
mobility <- read.csv("/Users/sameernair-desai/Desktop/COVID_Dominance/data/applemobilitytrends-2020-06-13.csv")
...
### Rashad Analysis - Generating the heterogenous multi-factor model.
...
###read in data
```{r}
cds_five<-read.csv("cds_daily_5y_13nov2020.csv",header=T,sep=',')
cds_five2<-read.csv("markit_CDS5Ydaily_13nov2020.csv",header=T,sep=',') ### markit CDS data to replace some bad data in the initial file (Greece)
ez_ten<-read.csv("cds_daily_10y_Ezone_dum.csv",header=T,sep=',')
...
```{r}
ez_cds2<-ez_cds
#ez_cds2$Greece[1:max(which(ez_cds2$Greece==14904.3600))]<-NA


d_ezcds<-apply(log(ez_cds2[,-1]),2,diff) #log differences of EZ spreads
d_necds<-apply(log(nonez_cds[,-1]),2,diff) # log differences of non-EZ spreads

glo_cds<-rowMeans(d_necds) # create a global CDS factor excluding EZ
ez_fac<-matrix(NA,nrow=nrow(d_ezcds),ncol=ncol(d_ezcds)) # create an EZ common factor excluding country i
for(i in 1:ncol(ez_fac)){
  ez_fac[,i]<-rowMeans(d_ezcds[,-i])
}


###training - test sample split
pre.dat<-d_ezcds[which(ez_cds2$Date[-1]<"2019-06-30"),]
post.dat<-d_ezcds[which(ez_cds2$Date[-1]>="2019-06-30"),]

glo_cds_pre<-glo_cds[which(ez_cds2$Date[-1]<"2019-06-30")]
glo_cds_post<-glo_cds[which(ez_cds2$Date[-1]>="2019-06-30")]
datez_post<-ez_cds2$Date[-1]
datez_post<-datez_post[which(datez_post>="2019-06-30")]
ez_fac_pre<-ez_fac[which(ez_cds2$Date[-1]<"2019-06-30"),]
ez_fac_post<-ez_fac[which(ez_cds2$Date[-1]>="2019-06-30"),]
...
tstatz<-matrix(NA,ncol=4,nrow=ncol(pre.dat))

predz<-matrix(NA,ncol=ncol(pre.dat),nrow=(nrow(post.dat)))
rsqz<-c(0)
for(i in 1:nrow(coefz)){
mod<-lm(pre.dat[,i]~Lag(pre.dat[,i],1)+glo_cds_pre+ez_fac_pre[,i])
coefz[i,]<-coef(mod)
tstatz[i,]<-summary(mod)$coefficients[,3]
predz[,i]<-coef(mod)[1]*rep(1,nrow(post.dat))+coef(mod)[2]*Lag(post.dat[,i],1)+coef(mod)[3]*glo_cds_post+coef(mod)[4]*ez_fac_post[,i]
rsqz[i]<-summary(mod)$r.squared
}

colnames(predz)<-colnames(post.dat)

### table with 1st stage results
reg_results<-ifelse(abs(tstatz[,-1])>2.58,paste(round(coefz[,-1],3),"***",sep=""),ifelse(abs(tstatz[,-1])>1.96,paste(round(coefz[,-1],3),"**",sep=""),ifelse(abs(tstatz[,-1])>1.645,paste(round(coefz[,-1],3),"*",sep=""),round(coefz[,-1],3))))

reg_results<-cbind(colnames(predz),reg_results,round(rsqz,2))
colnames(reg_results)<-c("Country","Lagged cds","global","regional","R-Squared")
#print.xtable(xtable(reg_results, 
#             caption="First-Stage Regression Results Estimated over January 2014 to June 2019"), 
#              "html", include.rownames=FALSE, caption.placement='top',
#               html.table.attributes='align="left"',file="firststage.html")



### correlations of model predictions with realized cds changes:
oos_rsquared<-c(00)
for(i in 1:ncol(post.dat)){
oos_rsquared[i]<-(cor(post.dat[,i],predz[,i],use="complete.obs")^2)
}
oos_rsquared[i+1]<-cor(rowMeans(post.dat),rowMeans(predz),use="complete.obs")^2
...
pdat<-data.frame(ez_cds2$Date[which(ez_cds2$Date>"2019-06-30")],rowMeans(post.dat),
                 rowMeans(post.dat[,which(colnames(post.dat) %in% c("Portugal","Italy","Ireland","Greece","Spain"))]),rowMeans(post.dat[,which(!colnames(post.dat) %in% c("Portugal","Italy","Ireland","Greece","Spain"))]),rowMeans(post.dat[,which(colnames(post.dat) %in% c("France","Germany","Belgium","Netherlands"))]),
                 rowMeans(post.dat[,which(colnames(post.dat) %in% c("Belgium","Spain","Italy","France","Netherlands"))]),rowMeans(post.dat[,which(!colnames(post.dat) %in% c("Slovakia","Latvia","Cyprus","Greece","Lithuania"))]),
                 rowMeans(predz),rowMeans(predz[,which(colnames(post.dat) %in% c("Portugal","Italy","Spain","Greece","Spain"))]),rowMeans(predz[,which(!colnames(post.dat) %in% c("Portugal","Italy","Ireland","Greece","Spain"))]),rowMeans(predz[,which(!colnames(post.dat) %in% c("France","Germany","Belgium","Netherlands"))]),
                 rowMeans(predz[,which(colnames(post.dat) %in% c("Belgium","Spain","Italy","France","Netherlands"))]),rowMeans(predz[,which(!colnames(post.dat) %in% c("Slovakia","Latvia","Cyprus","Greece","Lithuania"))]),
                 apply(apply(post.dat,2,cumsum),1,sd))
colnames(pdat)<-c("date","EZ_Avg","EZ_Avg_PIIGS","EZ_Avg_nPIIGS","EZ_Avg_Core","EZ_Avg_COVID","EZ_Avg_nCOVID","EZ_Pred","EZ_Pred_PIIGS","EZ_Pred_nPIIGS","EZ_Pred_Core","EZ_Pred_COVID","EZ_Pred_nCOVID","EZ_SD")

## Generating Figure 8A
...
p3a<-ggplot(data=pdat[-1,],aes(x=date,y=cumsum(EZ_Avg_PIIGS)))+geom_line()+geom_line(aes(y=cumsum(EZ_Avg_nPIIGS)),size=1)+theme_bw()+xlab("")+ylab("Cumulative Change (Log CDS)")+ggtitle("GIIPS Vs. Non-GIIPS: Actual")+geom_vline(xintercept=c(as.numeric(as.Date("2020-03-18")),as.numeric(as.Date("2020-06-04"))),alpha=.5)+ylim(c(-.6,1))+annotate("text", x = c(as.Date("2019-12-25"),as.Date("2020-01-05")), y = c(.1,-.5), label = c("non-GIIPS", "GIIPS") , fontface=c("bold","plain"))

#p3b<-ggplot(data=pdat[-1,],aes(x=date,y=cumsum(EZ_Pred_PIIGS)))+geom_line(linetype=2)+geom_line(aes(y=cumsum(EZ_Pred_nPIIGS)),linetype=2,size=1)+theme_bw()+xlab("")+ylab("Cumulative Change (Log CDS)")+ggtitle("GIIPS Vs. Non-GIIPS - Predicted")+geom_vline(xintercept=c(as.numeric(as.Date("2020-03-18")),as.numeric(as.Date("2020-06-04"))),alpha=.5)+ylim(c(-.6,1))+annotate("text", x = c(as.Date("2020-02-05"),as.Date("2020-01-05")), y = c(.1,-.4), label = c("non-GIIPS", "GIIPS") , fontface=c("bold","plain"))

## Generating Figure 9B
p3b<-ggplot(data=pdat[-1,],aes(x=date,y=cumsum(EZ_Avg_PIIGS-EZ_Pred_PIIGS)))+geom_line()+geom_line(aes(y=cumsum(EZ_Avg_nPIIGS-EZ_Pred_nPIIGS)),size=1)+theme_bw()+xlab("")+ylab("Cumulative Residuals")+labs(title="GIIPS Vs. Non-GIIPS: Actual-Fitted")+geom_vline(xintercept=c(as.numeric(as.Date("2020-03-18")),as.numeric(as.Date("2020-06-04"))),alpha=.5)+annotate("text", x = c(as.Date("2019-12-25"),as.Date("2020-01-10")), y = c(.1,-.1), label = c("Non-GIIPS", "GIIPS") , fontface=c("bold","plain"))

## Generating Figure 9C
...
## Generating Figure 9D
p5b<-ggplot(data=pdat[-1,],aes(x=date,y=cumsum(EZ_Avg_PIIGS-EZ_Pred_PIIGS)))+geom_line()+geom_line(aes(y=cumsum(EZ_Avg_Core-EZ_Pred_Core)),size=1)+theme_bw()+xlab("")+ylab("Cumulative Residuals")+labs(title="GIIPS Vs. Core: Actual-Fitted")+geom_vline(xintercept=c(as.numeric(as.Date("2020-03-18")),as.numeric(as.Date("2020-06-04"))),alpha=.5)+annotate("text", x = c(as.Date("2020-01-25"),as.Date("2020-01-10")), y = c(.1,-.1), label = c("Core", "GIIPS") , fontface=c("bold","plain"))

## Generating Figure 8C
p4a<-ggplot(data=pdat[-1,],aes(x=date,y=cumsum(EZ_Avg_COVID)))+geom_line()+geom_line(aes(y=cumsum(EZ_Avg_nCOVID)),size=1)+theme_bw()+xlab("")+ylab("Cumulative Change (Log CDS)")+ggtitle("High Vs. Low COVID Mortalities: Actual")+geom_vline(xintercept=c(as.numeric(as.Date("2020-03-18")),as.numeric(as.Date("2020-06-04"))),alpha=.5)+ylim(c(-.6,1))+annotate("text", x = c(as.Date("2019-12-25"),as.Date("2019-12-25")), y = c(.1,.75), label = c("Low Mortality", "High Mortality") , fontface=c("bold","plain"))

## Generating Figure 8D
p4b<-ggplot(data=pdat[-1,],aes(x=date,y=cumsum(EZ_Avg_COVID-EZ_Pred_COVID)))+geom_line()+geom_line(aes(y=cumsum(EZ_Avg_nCOVID-EZ_Pred_nCOVID)),size=1)+theme_bw()+xlab("")+ylab("Cumulative Residuals")+labs(title="High Vs. Low COVID Mortalities: Actual-Fitted")+geom_vline(xintercept=c(as.numeric(as.Date("2020-03-18")),as.numeric(as.Date("2020-06-04"))),alpha=.5)+annotate("text", x = c(as.Date("2020-01-01"),as.Date("2020-01-10")), y = c(.1,.5), label = c("Low Mortality", "High Mortality") , fontface=c("bold","plain"))

#p4b<-ggplot(data=pdat[-1,],aes(x=date,y=cumsum(EZ_Pred_COVID)))+geom_line(linetype=2)+geom_line(aes(y=cumsum(EZ_Pred_nCOVID)),linetype=2,size=1)+theme_bw()+xlab("")+ylab("Cumulative Change (Log CDS)")+ggtitle("COVID Vs. Non-COVID - Predicted")+geom_vline(xintercept=c(as.numeric(as.Date("2020-03-18")),as.numeric(as.Date("2020-06-04"))),alpha=.5)+ylim(c(-.6,1))+annotate("text", x = c(as.Date("2020-01-25"),as.Date("2020-01-25")), y = c(.1,.5), label = c("Low Mortality", "High Mortality") , fontface=c("bold","plain"))

grid.arrange(p,p2,p4a,p4b,nrow=2) #7x11
grid.arrange(p3a,p3b,p5a,p5b,nrow=2)

#write.csv(data.frame(pdat$date,post.dat),"covid_ez_spreads_15nov2020.csv")
#write.csv(data.frame(pdat$date,predz),"covid_ez_prediction_15nov2020.csv")
...
```{r}
###plot individual spreads
pdat<-data.frame(ez_cds2$Date[which(ez_cds2$Date>"2019-06-30")],post.dat,predz)
...
fstim<-read.csv("CESI_7.csv",header=T,sep=',')
###keep EU 
eu_countries<-c("Austria","Belgium","Cyprus","Estonia","Finland","France","Germany","Greece","Ireland","Italy","Latvia","Lithuania","Luxembourg","Malta","Netherlands","Portugal","Slovak Republic","Slovenia","Spain")
...
pd<-read.csv("EZ_PublicDebtToGDPYearly.csv",header=T,sep=',')
...
rates<-read.csv("_rMinusg.csv",header=T,sep=',')
...
pdat<-data.frame(pd3,rmg,p.cds.coefs.pd)
...
p2<-ggplot(data=pdat,aes(x=PD,y=Euro))+geom_point(shape=21,fill="grey",size=3)+theme_bw()+geom_text(aes(label=countries),hjust="inward", vjust="inward")+xlab("Public Debt/GDP (%)")+ylab("Regional Beta")+stat_cor(method = "pearson", label.x = 45, label.y = 1.9)+geom_smooth(method="lm",se=F,color="red",linetype=2,size=.5)

#p3<-ggplot(data=pdat,aes(x=PD,y=Euro+Global))+geom_point()+theme_bw()+geom_text(aes(label=countries),hjust="inward", vjust="inward")+xlab("Public Debt/GDP (%)")+ylab("Sum of Global+Regional Betas")#+geom_smooth(method="lm",se=F,color="red",linetype=2)+stat_cor(method = "pearson", label.x = 50, label.y = 1.5)

grid.arrange(p,p2,nrow=1)## 1000x400
pdat2<-data.frame(p.cds.coefs.stim)
colnames(pdat2)<-c("Country","Intercept","AR","Global","Euro","Fiscal")
...
p5<-ggplot(data=pdat2,aes(x=Euro,y=Fiscal))+geom_point(shape=21,size=3,fill="grey")+theme_bw()+geom_text(aes(label=Country),hjust="inward", vjust="inward")+ylab("COVID Stimulus/GDP (%)")+xlab("Regional Beta")+stat_cor(method = "pearson", label.x = 1, label.y = 20)+geom_smooth(method="lm",se=F,color="red",linetype=2,size=.5)
#p6<-ggplot(data=pdat2,aes(x=Euro+Global,y=Fiscal))+geom_point()+theme_bw()+geom_text(aes(label=Country),hjust="inward", vjust="inward")+ylab("COVID Stimulus/GDP (%)")+xlab("Sum of Global+Regional Beta")+geom_smooth(method="lm",se=F,color="red",linetype=2)+stat_cor(method = "pearson", label.x = 1.4, label.y = 20)
...
###grid.arrange(p7,p4,p5,nrow=1) ###1400x450


pdat3<-data.frame(names(pd3),pd3,colSums(post.dat[1:197,],na.rm=T)[-c(10,12)],colSums(post.dat[1:197,]-predz[1:197,],na.rm=T)[-c(10,12)])
...
pdat4<-data.frame(fstim,colSums(post.dat[1:197,],na.rm=T),colSums(post.dat[1:197,]-predz[1:197,],na.rm=T))
...
p11<-ggplot(data=pdat4,aes(y=covidresid,x=fstim))+geom_point(shape=21,fill="grey",size=3)+theme_bw()+stat_cor(method = "pearson", label.x =15, label.y = -.5)+geom_text(aes(label=country),hjust="inward",check_overlap = T)+ylab("Cumulative COVID Residual")+xlab("COVID Stimulus/GDP (%)")
  #plot(fstim$fiscal_7,colSums(predz,na.rm=T))
```
...
```{r}
cds_5y_actual_EZ <- aggregate(CDS_5y_Actual ~ Date, cds_5yr_merged, mean)
cds_5y_prediction_EZ <- aggregate(CDS_5y_Prediction ~ Date, cds_5yr_merged, mean)

eval <- c(0)

cds_5yr_EZ <- merge(cds_5y_actual_EZ, cds_5y_prediction_EZ, by = c("Date"), all.x = TRUE)
cds_5yr_EZ_preMar <- cds_5yr_EZ[which(cds_5yr_EZ$Date < '2020-03-01' & cds_5yr_EZ$Date >= '2020-01-01'),]
cds_5yr_EZ_preMar$residual2 <- (cds_5yr_EZ_preMar$CDS_5y_Actual - cds_5yr_EZ_preMar$CDS_5y_Predictio)^2  
eval[1] <- mean(cds_5yr_EZ_preMar$residual2, na.rm = TRUE)

cds_5yr_EZ_postMar <- cds_5yr_EZ[which(cds_5yr_EZ$Date >= '2020-04-01' & cds_5yr_EZ <= '2020-05-31'),]
cds_5yr_EZ_postMar$residual2 <- (cds_5yr_EZ_postMar$CDS_5y_Actual - cds_5yr_EZ_postMar$CDS_5y_Predictio)^2  
eval[2] <- mean(cds_5yr_EZ_postMar$residual2, na.rm = TRUE)

cds_5yr_EZ_Mar <- cds_5yr_EZ[which(cds_5yr_EZ$Date >= '2020-03-01' & cds_5yr_EZ$Date < '2020-04-01'),]
cds_5yr_EZ_Mar$residual2 <- (cds_5yr_EZ_Mar$CDS_5y_Actual - cds_5yr_EZ_Mar$CDS_5y_Predictio)^2  
eval[3] <- mean(cds_5yr_EZ_Mar$residual2, na.rm = TRUE)

p_EZ_preMar <- ggplot(dat = cds_5yr_EZ_preMar,aes(x=Date,y=CDS_5y_Actual,linetype="Actual"))+geom_line()+geom_line(aes(y=CDS_5y_Prediction,linetype="Fitted"))+theme_bw()+xlab("")+ylab("Daily Change (Log CDS)")+ggtitle("Eurozone Average CDS Spreads, Mid Dec 2019 - Feb 2020")+theme(axis.title.y = element_text(size = 10),axis.text = element_text(size = 10),legend.title = element_blank(),plot.title = element_text(size=12),legend.position = c(0.8, 0.8))+ylim(-0.15,0.2)

p_EZ_postMar <- ggplot(dat = cds_5yr_EZ_postMar,aes(x=Date,y=CDS_5y_Actual,linetype="Actual"))+geom_line()+geom_line(aes(y=CDS_5y_Prediction,linetype="Fitted"))+theme_bw()+xlab("")+ylab("Daily Change (Log CDS)")+ggtitle("Eurozone Average CDS Spreads, Apr 2020 - Mid June 2020")+theme(axis.title.y = element_text(size = 10),axis.text = element_text(size = 10),legend.title = element_blank(),plot.title = element_text(size=12),legend.position = c(0.8, 0.8))+ylim(-0.15,0.2)

p_EZ_Mar <- ggplot(dat = cds_5yr_EZ_Mar,aes(x=Date,y=CDS_5y_Actual,linetype="Actual"))+geom_line()+geom_line(aes(y=CDS_5y_Prediction,linetype="Fitted"))+theme_bw()+xlab("")+ylab("Daily Change (Log CDS)")+ggtitle("Eurozone Average CDS Spreads, March 2020")+theme(axis.title.y = element_text(size = 10),axis.text = element_text(size = 10),legend.title = element_blank(),plot.title = element_text(size=12),legend.position = c(0.8, 0.8))+ylim(-0.15,0.2)


## Generating Figure 10
p_EZ <- grid.arrange(p_EZ_preMar, p_EZ_Mar, p_EZ_postMar, ncol=3,nrow=1)
```
...
new_res.mortality.4 <- plm(CDS_5y_Residual ~ Lag(New_Mortality_Rate) + Lag(New_Mortality_Rate_Growth) + Lag(Total_Mortality_Rate) + Lag(Total_Mortality_Rate_Growth) + driving + SI_Growth + Lag(Dummy_Fiscal_Country) + Lag(Dummy_Fiscal_EU) + Lag(Dummy_Monetary_ECB) + Lag(Dummy_Monetary_Fed),
                                      method="pooling", effect="twoways",
                                      data=panel_inCOVID[which(!is.infinite(-panel_inCOVID$New_Mortality_Rate_Growth) & !is.infinite(-panel_inCOVID$Total_Mortality_Rate_Growth)),], na.action="na.exclude")
...
stargazer(digits=4,new_res.mortality.2,new_res.mortality.3,new_res.mortality.4,
          type="html",se=list(se.new_res.mortality.2[,2],se.new_res.mortality.3[,2],se.new_res.mortality.4[,2]), out=file.path("Table_inCOVID_panel_output_newRES_mortality.htm"),
          ...
          covariate.labels=c("New Mortality Rate", "New Mortality Rate Growth", 
                             ...
                             "Mobility", "SI Growth",
                             "Country Fiscal Policy Dummy", "EU Fiscal Policy Dummy", "ECB Policy Dummy", "Fed Policy Dummy"), df = FALSE, omit.stat="adj.rsq", 
...
new_CDS.mortality.4 <- plm(CDS_5y_Actual ~ CDS_5y_Prediction + Lag(New_Mortality_Rate) + Lag(New_Mortality_Rate_Growth) + Lag(Total_Mortality_Rate) + Lag(Total_Mortality_Rate_Growth) + driving + SI_Growth + Lag(Dummy_Fiscal_Country) + Lag(Dummy_Fiscal_EU) + Lag(Dummy_Monetary_ECB) + Lag(Dummy_Monetary_Fed),
                                      method="pooling", effect="twoways",
                                      data=panel_inCOVID[which(!is.infinite(-panel_inCOVID$New_Mortality_Rate_Growth) & !is.infinite(-panel_inCOVID$Total_Mortality_Rate_Growth)),], na.action="na.exclude")
...
stargazer(digits=4,new_CDS.mortality.0,new_CDS.mortality.2,new_CDS.mortality.3,new_CDS.mortality.4,
          type="html",se=list(se.new_CDS.mortality.0[,2],se.new_CDS.mortality.2[,2],se.new_CDS.mortality.3[,2],se.new_CDS.mortality.4[,2]),out=file.path("Table_inCOVID_panel_output_newCDS_mortality.htm"),
...
new_CDS.mortality.4.small <- plm(CDS_5y_Actual ~ CDS_5y_Prediction + Lag(New_Mortality_Rate) + Lag(New_Mortality_Rate_Growth) + Lag(Total_Mortality_Rate) + Lag(Total_Mortality_Rate_Growth) + driving + SI_Growth + Lag(Dummy_Fiscal_Country) + Lag(Dummy_Fiscal_EU) + Lag(Dummy_Monetary_ECB) + Lag(Dummy_Monetary_Fed),
                                      method="pooling", effect="twoways",
                                      data=panel_inCOVID_small, na.action="na.exclude")
...
stargazer(digits=4,new_CDS.mortality.0.small,new_CDS.mortality.2.small,new_CDS.mortality.3.small,new_CDS.mortality.4.small,
          type="html",se=list(se.new_CDS.mortality.0.small[,2],se.new_CDS.mortality.2.small[,2],se.new_CDS.mortality.3.small[,2],se.new_CDS.mortality.4.small[,2]),out=file.path("Table_inCOVID_panel_output_newCDS_mortality_small.htm"),
          dep.var.labels=c("Daily CDS Spread Change"),
          covariate.labels=c("Fitted Daily CDS Spread Change", "New Mortality Rate", "New Mortality Rate Growth", 
                             "Total Mortality Rate", "Total Mortality Rate Growth",
                             "Mobility", "SI Growth",
...
### EZ aggregate: realized values versus model-implied values
```{r}
...
panel_inCOVID_EZ_prediction <- aggregate(CDS_5y_Prediction ~ Date, panel_inCOVID_small, mean)
panel_inCOVID_EZ_actual <- aggregate(CDS_5y_Actual ~ Date, panel_inCOVID_small, mean)
panel_inCOVID_EZ_prediction_new <- aggregate(CDS_5y_Prediction_New ~ Date, panel_inCOVID_small, mean)

panel_inCOVID_EZ <- merge(panel_inCOVID_EZ_prediction_new, panel_inCOVID_EZ_prediction, by = c("Date"), all.x = TRUE)
panel_inCOVID_EZ <- merge(panel_inCOVID_EZ, panel_inCOVID_EZ_actual, by = c("Date"), all.x = TRUE)


## Generating Figure 11
p_EZ_Mar_New <- ggplot(dat = panel_inCOVID_EZ,aes(x=Date,y=CDS_5y_Actual,linetype = "Actual"))+geom_line()+geom_line(aes(y=CDS_5y_Prediction, linetype="Fitted by model [1]"))+
  geom_line()+geom_line(aes(y=CDS_5y_Prediction_New,linetype="Fitted by model [4]"))+theme_bw()+xlab("")+ylab("Daily Change (Log CDS)")+
  ggtitle("Eurozone Average CDS Spreads, March 2020")+
